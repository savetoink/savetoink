...
func (h *Handlers) HandleCreateArticle(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.Tags) > 0 {
		if len(req.Tags) > consts.MaxTagsPerArticle {
			utils.WriteJSONError(
				w,
				http.StatusBadRequest,
				fmt.Errorf("maximum %d tags allowed per article", consts.MaxTagsPerArticle),
			)
			return
		}
	}
...
func (h *Handlers) validateAndCheckQuota(
	...
	if req.SendOnComplete {
		...
		sendsError, err := utils.CheckQuotaAndDeviceEmail(
			r.Context(), w, r, h.service, h.cfg.AuthBackend, accountID, h.cfg.DisableQuotaCheck,
		)
		if err != nil {
...
func (h *Handlers) HandleSendArticle(w http.ResponseWriter, r *http.Request) {
	...
	sendsCount, err := utils.CheckQuotaAndDeviceEmail(
		r.Context(), w, r, h.service, h.cfg.AuthBackend, accountID, h.cfg.DisableQuotaCheck,
	)
	if err != nil {
		return
