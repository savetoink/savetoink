...
func (s *DynamoDBRepositoryTestSuite) TestAddTagsToArticleWithCreatedAt() {
	ctx := context.Background()
	t := s.T()
...
func (s *DynamoDBRepositoryTestSuite) TestAddTagsToArticle_BothMethodsSameResult() {
	ctx := context.Background()
	t := s.T()
	...
	require.NoError(t, err)
	err = s.repositories.Store(ctx, article2)
	require.NoError(t, err)
...
func (s *DynamoDBRepositoryTestSuite) TestAddTagsToArticle_EmptyTags() {
	ctx := context.Background()
	t := s.T()
	...
	err = s.repositories.AddTagsToArticle(ctx, article.Account, article.ID, []string{}, nil)
	require.NoError(t, err)
...
func (s *DynamoDBRepositoryTestSuite) TestAddTagsToArticle_NonExistentArticle() {
	ctx := context.Background()
	t := s.T()
...
func (s *DynamoDBRepositoryTestSuite) TestAddTagsToArticle_DuplicateTags() {
	ctx := context.Background()
	t := s.T()
...
func (s *DynamoDBRepositoryTestSuite) TestRemoveTagsFromArticle() {
	ctx := context.Background()
	t := s.T()
...
func (s *DynamoDBRepositoryTestSuite) TestRemoveTagsFromArticle_EmptyTags() {
	ctx := context.Background()
	t := s.T()
	...
	err = s.repositories.RemoveTagsFromArticle(ctx, article.Account, article.ID, []string{})
	require.NoError(t, err)
}

func (s *DynamoDBRepositoryTestSuite) TestRemoveTagsFromArticle_NonExistentArticle() {
	ctx := context.Background()
	t := s.T()
...
func (s *DynamoDBRepositoryTestSuite) TestSetArticleTags() {
	ctx := context.Background()
	t := s.T()
...
func (s *DynamoDBRepositoryTestSuite) TestSetArticleTags_EmptyTags() {
	ctx := context.Background()
	t := s.T()
	...
	err = s.repositories.SetArticleTags(ctx, article.Account, article.ID, []string{})
	require.NoError(t, err)
...
func (s *DynamoDBRepositoryTestSuite) TestGetArticleTags() {
	ctx := context.Background()
	t := s.T()
	...
	tags, err := s.repositories.GetArticleTags(ctx, article.Account, article.ID)
	require.NoError(t, err)
	assert.Empty(t, tags)
...
func (s *DynamoDBRepositoryTestSuite) TestGetArticlesByTag() {
	ctx := context.Background()
	t := s.T()
	...
	for _, art := range articles {
		...
		err := s.repositories.Store(ctx, article)
		require.NoError(t, err)
		err = s.repositories.AddTagsToArticle(ctx, article.Account, article.ID, art.tags, nil)
		require.NoError(t, err)
	}
...
func (s *DynamoDBRepositoryTestSuite) TestGetArticlesByTag_Pagination() {
	...
	for i := range articleCount {
		...
		err := s.repositories.Store(ctx, article)
		require.NoError(t, err)
		err = s.repositories.AddTagsToArticle(ctx, article.Account, article.ID, []string{"pagetag"}, nil)
		require.NoError(t, err)
	}

	// First page
	ids, total, err := s.repositories.GetArticlesByTag(ctx, dynamoDBTestAccount, "pagetag", 1, 2)
	require.NoError(t, err)
	assert.Equal(t, articleCount, total)
	assert.Len(t, ids, 2)

	// Second page
	ids, total, err = s.repositories.GetArticlesByTag(ctx, dynamoDBTestAccount, "pagetag", 2, 2)
	require.NoError(t, err)
	assert.Equal(t, articleCount, total)
	assert.Len(t, ids, 2)

	// Third page (last page)
	ids, total, err = s.repositories.GetArticlesByTag(ctx, dynamoDBTestAccount, "pagetag", 3, 2)
	require.NoError(t, err)
	assert.Equal(t, articleCount, total)
	assert.Len(t, ids, 1)
}

func (s *DynamoDBRepositoryTestSuite) TestGetAllTagsForAccount() {
	ctx := context.Background()
	t := s.T()
...
func (s *DynamoDBRepositoryTestSuite) TestDeleteTagsForArticle() {
	ctx := context.Background()
	t := s.T()
	...
	// Delete all tags
	err = s.repositories.DeleteTagsForArticle(ctx, article.Account, article.ID)
	require.NoError(t, err)
...
func (s *DynamoDBRepositoryTestSuite) TestDeleteTagsForArticle_NoTags() {
	ctx := context.Background()
	t := s.T()
...
func (s *DynamoDBRepositoryTestSuite) TestTagIsolation() {
	ctx := context.Background()
	t := s.T()
